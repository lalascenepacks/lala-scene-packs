...
import AnimeCharactersAccordion from "./AnimeCharactersAccordion";
...
export default function AnimeFilterClient({
  anime,
  packs,
}: {
  anime: string;
  ...
  const normalizedSearch = normalizeCharacter(searchedCharacter);

  const entry = Object.entries(grouped).find(([character]) => {
    const normalizedCharacter = normalizeCharacter(character);

    return (
      normalizedCharacter === normalizedSearch ||
      normalizedCharacter.includes(normalizedSearch) ||
      normalizedSearch.includes(normalizedCharacter)
    );
  });
        ...
        >
          ...
          >
            ...
            {totalCharacters === 0 ? (
              ...
              >
                No characters found for this anime with these filters.
              </div>
            ) : (
              <AnimeCharactersAccordion grouped={visibleGrouped} />
            )}
          </section>
        </div>
      </div>
