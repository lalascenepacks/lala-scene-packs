...
function getSubtitle(fileName: string) {
  ...
  if (partMatch) {
    return `${partMatch[0].replace(/parte/i, "Parte")}${userMatch ? ` ${userMatch[0]}` : ""}`;
  }

  if (episodeMatch) {
    return `${episodeMatch[0].toUpperCase()}${userMatch ? ` ${userMatch[0]}` : ""}`;
  }
...
export default function AnimeCharactersAccordion({
  grouped,
}: {
              ...
              <div>
                {packs.map((item) => (
                  <div
                    ...
                    style={{
                      display: "grid",
                      gridTemplateColumns: "1.8fr 110px 140px 120px 140px",
                      padding: "18px 20px",
