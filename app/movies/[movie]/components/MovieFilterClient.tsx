...
import MovieCharactersAccordion from "./MovieCharactersAccordion";
...
function getCurrentHashCharacter() {
  if (typeof window === "undefined") return "";
  return normalizeCharacter(window.location.hash.replace("#", ""));
}
...
export default function MovieFilterClient({
  movie,
  packs,
}: {
  movie: string;
  ...
  useEffect(() => {
    function syncHashCharacter() {
      setSearchedCharacter(getCurrentHashCharacter());
    }
  ...
  const visibleGrouped = useMemo(() => {
    ...
    const entry = Object.entries(grouped).find(([character]) => {
      const normalized = normalizeCharacter(character);
      return (
        normalized === searchedCharacter ||
        normalized.includes(searchedCharacter) ||
        searchedCharacter.includes(normalized)
      );
    });
        ...
        >
          ...
          >
            ...
            {totalCharacters === 0 ? (
              ...
              >
                No characters found for this movie with these filters.
              </div>
            ) : (
              <MovieCharactersAccordion grouped={visibleGrouped} />
            )}
          </section>
        </div>
      </div>
